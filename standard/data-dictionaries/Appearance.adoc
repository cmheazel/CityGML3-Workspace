...
!{set:cellbgcolor:#FFFFFF} imageURI «Property»  !<<URI-section,URI>> [1..1] !Specifies the URI that points to the external image data file.
...
!{set:cellbgcolor:#FFFFFF} target «Property»  !<<URI-section,URI>> [1..1] !Specifies the URI that points to the surface geometry object to which the texture is applied.
!===
|{set:cellbgcolor:#FFFFFF} Note: Unless otherwise specified, all attributes and roll names have the stereotype «Property».
|=== 
