[[ats_LandUse_LandUseClassValue]]
[width="90%",cols="2,6a"]
|===
^|*Abstract Test {counter:ats-id}* |*/ats/LandUse/LandUseClassValue* 
^|Test Purpose |To validate that the Implementation Specification correctly implements the UML Codelists defined in the Conceptual Model
^|Requirement |<<req_LandUse_LandUseClassValue,/req/LandUse/rc-LandUseClassValue>>
^|Test Method |Manual Inspection
^|A |Validate that all instances of the LandUseClassValue codelist in the Implementation Specification can only take the values specified for that codelist in the Conceptual Model 
|===
